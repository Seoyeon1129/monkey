...
# 사용자 입력 텍스트
if language == "한국어":
    input_text = st.text_input("한글 텍스트를 입력하세요:", value="")
    valid_characters = korean_characters
else:
    input_text = st.text_input("영문 텍스트를 입력하세요 (대소문자 알파벳, 숫자, 마침표, 쉼표, 느낌표, 물음표, 겹따옴표, 홑따옴표, 괄호, 콜론, 세미콜론만 사용 가능):", value="")
...
def start_generation():
    ...
    while st.session_state.running:
        ...
        # 생성된 텍스트와 문자 개수 표시
        st.text_area("생성된 텍스트", st.session_state.generated_text, height=200)
        st.write(f"현재까지 입력된 문자 개수: {st.session_state.char_count}")
